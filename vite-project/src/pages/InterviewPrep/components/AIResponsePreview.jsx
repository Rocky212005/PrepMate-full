      ...
      <div className='text-[14px] prose prose-slate dark:prose-invert max-w-none'>
        <ReactMarkdown
          ...
          components={{
            ...
            h1({children}){
                return <h1 className='text-2xl  font-bold mt-5 mb-4'>{children}</h1>
            },
             h2({children}){
             ...
             td({children}){
                return <td className='px-3 pt-2 whitespace-nowrap text-sm'>{children}</td>
            },
             th({children}){
    ...
    return <div className='relative my-6 rounded-lg overflow-hidden bg-gray-50 border border-gray-200'>
        <div className='flex items-center justify-between px-4 py-2 bg-gray-100 border-b border-gray-200'>
            ...
            <button onClick={copyCode} className='text-gray-500 hover:text-gray-700 focus:outline-none relative group'  aria-label='Copy code'>
                ...
                :(<LuCheck size={16}/>)}
                {copied && (
                    <span className=' absolute -top-8 right-0 bg-black text-white text-xs rounded-md px-2 py-1 opacity-80 group-hover:opacity-100 transition '>Cpired!</span>
                )}
            </button>
        </div>
        <SyntaxHighlighter  
